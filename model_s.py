...
def token_model(end_day, rg, Decay, p, w, c, tf, br, y1, y2, lps, gps):
    ...
    #df = pd.DataFrame({'day': range(1,end_day,1), 'registration': range(1,end_day,1)})
    ...
    #change the below formula to change the trend in registration
    ...
    end_day_value = df.Total_Supply[end_day-2]
    
    return end_day_value
